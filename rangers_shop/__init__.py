...
jwt = JWTManager(app) # makes token for API
...
app.json_encoder = json_encoder #we are not instantiating an obkject we are simply pointing to this class we made to encode data objects
cors = CORS(app) #Cross Origin Resource Sharing aka allowing other apps to talk to app/server
